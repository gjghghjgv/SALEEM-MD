...
// Owner numbers
const ownerNumber = ["923022147740"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    console.log("✅ saleem.html found in lib folder");
  } else {
    console.log("⚠️ saleem.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("SALEEM~")) {
    console.log("❌ Invalid SALEEM session format");
    process.exit(1);
  }
  ...
  console.log("♻️ Saleem session restored successfully");
}
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923022147740@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
